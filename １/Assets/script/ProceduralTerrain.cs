...
public class ProceduralTerrain : MonoBehaviour
{
    ...
    public float wallHeight = 20f;   // 壁の高さ
    public float wallThickness = 1f; // 壁の厚み
    ...
    void SpawnObjects()
    {
        bool spawnClear = (Random.value < 0.3f); // 20％で true
    ...
    void SpawnWeapon()
    {
        { if (weaponPrefabs == null || weaponPrefabs.Length == 0) return;
            ...
            float x = Random.Range(0, width);
            float z = Random.Range(0, depth);
            float y = GetHeightAtPosition(x, z) + 0.2f; Vector3 pos = new Vector3(x, y, z) + transform.position; spawnedWeapon = Instantiate(weaponPrefab, pos, Quaternion.identity);
        }
        // 他のメソッド（SpawnObjects, SpawnEnemies など）は変更なし
    }
    void CreateWall(Vector3 localPos, Vector3 size, string name)
    {
    ...
    void CreateInvisibleWalls()
    {
        ...
        {
            if (child.name.StartsWith("InvisibleWall"))
                Destroy(child.gameObject);
        }

        float terrainWidth = width;
        float terrainDepth = depth;
        float centerY = wallHeight / 2f;

        
        CreateWall(
            new Vector3(terrainWidth / 2f, centerY, terrainDepth + wallThickness / 2f),
            new Vector3(terrainWidth, wallHeight, wallThickness),
            "InvisibleWall_North"
        );

        
        CreateWall(
            new Vector3(terrainWidth / 2f, centerY, -wallThickness / 2f),
            new Vector3(terrainWidth, wallHeight, wallThickness),
            "InvisibleWall_South"
        );
        ...
        CreateWall(
            new Vector3(terrainWidth + wallThickness / 2f, centerY, terrainDepth / 2f),
            new Vector3(wallThickness, wallHeight, terrainDepth),
            "InvisibleWall_East"
        );

        
        CreateWall(
            new Vector3(-wallThickness / 2f, centerY, terrainDepth / 2f),
            new Vector3(wallThickness, wallHeight, terrainDepth),
            "InvisibleWall_West"
        );
    }
